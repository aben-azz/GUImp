...
/*   Updated: 2019/10/24 11:54:36 by aben-azz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int					print_numbers(int fd, t_fmt *fmt, char *str, int len)
{
	...
	if (fmt->opt & SUB)
	{
		...
		ft_nputchar_fd('0', fmt->prec, fd);
		ft_putstr_fd(str, fd);
		ft_nputchar_fd(' ', len, fd);
	}
	else
	{
		(~fmt->opt & ZERO) && ft_nputchar_fd(' ', len, fd);
		fmt->signe && ft_putchar_fd(fmt->signe, fd);
		(fmt->prefixe) && (ret = print_prefixe(fd, fmt->type));
		(fmt->opt & ZERO) && ft_nputchar_fd('0', len, fd);
		ft_nputchar_fd('0', fmt->prec, fd);
		ft_putstr_fd(str, fd);
	}
	len = ft_max(len, 0);
